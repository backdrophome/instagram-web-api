...
class Instagram {
  ...
  async getMediaByShortcode({ shortcode }) {
    return this.request(`/p/${shortcode}/?__a=1`).then(data => {
      if (data && data.items) {
        const mediaUrls = data.items.map(d => {
          const sources = d.video_versions
            ? [d.video_versions[0]].map(v => ({
                url: v.url,
                user: {
            ...
            : [
                {
                  url: d.image_versions2.candidates[0].url,
                  user: {
                    username: d.user.username,
                    full_name: d.user.full_name
                  },
